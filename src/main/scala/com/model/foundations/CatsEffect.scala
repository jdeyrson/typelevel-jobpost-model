...
import cats.effect.kernel.{ Concurrent, Fiber, Ref, Spawn, Sync }
import cats.effect.{ Deferred, GenSpawn, IO, IOApp, MonadCancel, Resource, Temporal }
...
object CatsEffect extends IOApp.Simple {
  ...
  val pritedMeaningOfLife   = firstIO.flatMap(mol => IO(println(mol)))
  // for-comprehensions
  ...
  val delayedPring = IO.sleep(1.second) *> IO(println(Random.nextInt(100)))
  val manyPrints = for {
    fib1 <- delayedPring.start
    fib2 <- delayedPring.start
    _    <- fib1.join
    _    <- fib2.join
  } yield ()

  val cancelledFiber = for {
    fib <- delayedPring.onCancel(IO(println("I'm cancelled!"))).start
  ...
  val ignoredCancellation = for {
    fib <- IO.uncancelable(_ => delayedPring.onCancel(IO(println("I'm cancelled")))).start
    _   <- IO.sleep(500.millis) *> IO(println("cancelling fiber")) *> fib.cancel
    _   <- fib.join
  } yield ()
  ...
  val fiber = spawnIO.start(delayedPring) // creates a fiber, same as delayedPrint.start

  // Concurrent = concurrency primitives (atomic references + promises)
  trait MyConcurrent[F[_]] extends Spawn[F] {
    def ref[A](a: A): F[Ref[F, A]]
    def referred[A]: F[Deferred[F, A]]
  }
  ...
  // Async = ability to suspend asynchronous computations (i.e. on other thread pools) into an effect manafed by CE
